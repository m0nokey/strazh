...
install_host_cli() (
    ...
    [[ -r "$STRAZH_HOST_CLI_SOURCE" ]] ||
        die "Host utility source is missing: $STRAZH_HOST_CLI_SOURCE"
    install -d -m 0755 -o root -g root "$(dirname "$STRAZH_HOST_CLI_DST")"
    tmp="$(mktemp "${STRAZH_HOST_CLI_DST}.tmp.XXXXXX")" ||
        die "Cannot create temporary host utility: $STRAZH_HOST_CLI_DST"
    trap 'rm -f -- "$tmp"' EXIT

    install -m 0750 -o root -g root "$STRAZH_HOST_CLI_SOURCE" "$tmp" ||
        die "Cannot stage host utility: $STRAZH_HOST_CLI_DST"
    bash -n "$tmp" || die "Host utility failed syntax validation"
    mv -f -- "$tmp" "$STRAZH_HOST_CLI_DST" ||
        die "Cannot publish host utility: $STRAZH_HOST_CLI_DST"
    trap - EXIT
...
main() {
    ...
    # Install the host administration command while the project is present.
